...
const prescriptionForm = $("#prescriptionForm");   // 진단서 폼 태그
...
function validation(){
    const dansResponseInput = $("#dansResponseInput");    // 의사소견 입력창
    const dansCostInput = $("#dansCostInput");    // 진료비 입력창

    let check = /^[0-9]+$/; // 숫자만 입력했는지 체크

    if(dansResponseInput.val() == ""){
        alert("의사 소견을 입력해주세요");
        dansResponseInput.focus();
        return false;
    }

    if(dansCostInput.val() == ""){
        alert("진료비를 입력해주세요");
        dansCostInput.focus();
        return false;
    }

    if(!check.test(dansCostInput.val())){
